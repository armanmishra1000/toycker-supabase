...
export const getGridClassName = (mode: ViewMode) => {
  if (mode === "grid-5") {
    return "grid w-full grid-cols-1 gap-x-5 gap-y-8 xsmall:grid-cols-2 small:grid-cols-3 medium:grid-cols-5"
  }

  if (mode === "grid-4") {
    return "grid w-full grid-cols-1 gap-x-6 gap-y-10 xsmall:grid-cols-2 small:grid-cols-3 medium:grid-cols-4"
  }
